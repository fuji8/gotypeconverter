...
func (g *Generator) Generate(srcStructName, dstStructName string) ([]byte, error) {
	if g.top == nil {
		return nil, fmt.Errorf("parse() first")
	}
	...
	if g.src == nil || g.dst == nil {
		return nil, fmt.Errorf("src or dst is nil")
	}
...
func (g *Generator) Init(fileName string) (err error) {
	...
	f, ok := g.top.(*ast.File)
	if !ok {
		return fmt.Errorf("top is not ast.File")
	}
